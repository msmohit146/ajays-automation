...
print("Starting doctr document layout analyzer...", flush=True)
sys.stdout.flush()
...
try:
    ...
    print("doctr OCR model loaded successfully!", flush=True)
    use_doctr = True
except Exception as e:
    print(f"ERROR loading doctr: {e}", flush=True)
...
def extract_text_doctr(image_path, model):
    """Extract text using doctr with layout analysis"""
    try:
        # doctr processes image and returns document structure
        doc = DocumentFile.from_pdf(image_path) if image_path.lower().endswith('.pdf') else DocumentFile.from_image(image_path)

        # Perform OCR with layout detection
        result = model(doc)

        # Extract text while preserving layout
        texts = []

        # doctr returns pages with blocks
...
try:
    ...
    for idx, (full_path, file, folder) in enumerate(images_to_process):
        ...
        try:
            if use_doctr:
                text = extract_text_doctr(full_path, model)
            else:
                text = extract_text_easyocr(full_path, reader)
        except Exception as e:
    ...
    print(f"COMPLETE! All {total_images} files indexed with doctr layout analysis.", flush=True)
    sys.stdout.flush()

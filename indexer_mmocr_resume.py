...
print("Starting MMocr layout-aware indexer with RESUME support...", flush=True)
sys.stdout.flush()

try:
    print("Loading MMocr OCR model...", flush=True)
    sys.stdout.flush()
    ...
    print("MMocr loaded successfully!", flush=True)
    use_mmocr = True
except Exception as e:
    print(f"ERROR loading MMocr: {e}", flush=True)
...
def extract_text_mmocr(image_path, ocr):
    """Extract text using MMocr"""
    try:
        ...
        for pred in predictions:
            if 'text' in pred and pred.get('scores', 0) > 0.2:
        ...
        print(f"ERROR in MMocr for {image_path}: {e}", flush=True)
        return ""
...
try:
    ...
    for idx, (full_path, file, folder) in enumerate(images_to_process):
        ...
        try:
            if use_mmocr:
                text = extract_text_mmocr(full_path, ocr)
            else:
    ...
    print(f"COMPLETE! All {total_images} files indexed with MMocr layout detection.", flush=True)
    sys.stdout.flush()
